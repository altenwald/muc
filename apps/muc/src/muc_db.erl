...
list_rooms(JID) ->
    {ok,_Count,Rows} = dbi:do_query(?POOLDB, ?LIST_ROOMS, [JID]),
    Rows.
...
get_room_info(JID, Room) ->
    case dbi:do_query(?POOLDB, ?GET_ROOM_INFO_SEC, [JID, Room]) of
        {ok,_Count,[Row]} ->
...
get_room_info(Room) ->
    case dbi:do_query(?POOLDB, ?GET_ROOM_INFO, [Room]) of
        {ok,_Count,[Row]} ->
            {ok, room_fields_map(Row)};
        {ok,0,[]} ->
            {error, notfound};
...
save_room(JID, Description, Subject) ->
    ...
    case dbi:do_query(?POOLDB, ?SAVE_ROOM, [Name, JID, Description, Subject]) of
        {ok,1,[{Id}]} -> 
            {ok, #room_info{
...
save_user(JID, #room_user{
    ...
    R = from_role(Role),
    case dbi:do_query(?POOLDB, ?SAVE_USER, [JID, User, A, R, Nick]) of
        {ok,1,[{Id}]} ->
...
get_users(Room) ->
    case dbi:do_query(?GET_USERS, [Room]) of
        {ok,_Count,Users} ->
...
update_room(RoomInfo) ->
    ...
    case dbi:do_query(?POOLDB, ?UPDATE_ROOM, Params) of
        {ok,1,_} -> {ok, RoomInfo};
